...
public class ThirstManager {
    ...
    public void update(PlayerEntity player) {
        if (unpurifiedQueue) {
            if (Math.random() > 0.5) {
                player.damage(
                        CustomDamageSources.of(player.getWorld(), CustomDamageSources.THIRST),
                        Tempus.config.unpurifiedDamageAmount);
                player.addStatusEffect(new StatusEffectInstance(
                        StatusEffects.HUNGER,
                        Tempus.config.unpurifiedHungerStatusDuration,
                        Tempus.config.unpurifiedHungerStatusAmplifier));
            }
        ...
        if (thirstTickTimer >= thirstTickThreshold) {
            ...
            if (thirstLevel <= 0)
                player.damage(
                        CustomDamageSources.of(player.getWorld(), CustomDamageSources.THIRST),
                        Tempus.config.thirstDamageAmount);
            else {
                thirstLevel--;
