...
public class NutritionCommand implements CommandRegistrationCallback {
    ...
    private static int executeQuery(ServerCommandSource source, Collection<ServerPlayerEntity> players) {
        for (PlayerEntity player : players) {
            NutritionManager manager =
                    ((NutritionManager.MixinAccessor) player.getHungerManager()).tempus$getNutritionManager();
            source.sendFeedback(
                    () -> nutritionToText(
                            Text.translatable("command.tempus.nutrition.query", player.getDisplayName()),
                            manager.getNutritionType(),
                            manager.getNutritionLevel()),
                    true);
        }
    ...
    private static int executeSet(
            ServerCommandSource source, NutritionType type, int level, Collection<ServerPlayerEntity> players) {
        for (PlayerEntity player : players) {
            ((NutritionManager.MixinAccessor) player.getHungerManager())
                    .tempus$getNutritionManager()
                    .setState(type, level);
            ((NutritionManager.MixinAccessor) player.getHungerManager())
                    .tempus$getNutritionManager()
                    .syncNutrition(player);
        }
        if (players.size() == 1) {
            ...
            source.sendFeedback(
                    () -> nutritionToText(
                            Text.translatable("command.tempus.nutrition.set.single", player.getDisplayName()),
                            manager.getNutritionType(),
                            manager.getNutritionLevel()),
                    true);
        } else {
            source.sendFeedback(
                    () -> nutritionToText(
                            Text.translatable("command.tempus.nutrition.set.multiple", players.size())
                                    .copy(),
                            type,
                            level),
                    true);
        ...
        dispatcher.register(literal("nutrition")
                ...
                .then(literal("clear")
                        .executes(context -> executeClear(
                                context.getSource(), List.of(context.getSource().getPlayerOrThrow())))
                        .then(argument("targets", EntityArgumentType.players())
                                .executes(context -> executeClear(
                                        context.getSource(),
                                        context.getArgument("targets", EntitySelector.class)
                                                .getPlayers(context.getSource())))))
                .then(literal("query")
                        ...
                        .then(argument("nutrition", new NutritionArgumentType())
                                .then(argument("level", IntegerArgumentType.integer(1, 10))
                                        .executes(context -> executeSet(
                                                context.getSource(),
                                                context.getArgument("nutrition", NutritionType.class),
    ...
    static class NutritionArgumentType implements ArgumentType<NutritionType> {
        ...
        public NutritionType parse(StringReader reader) throws CommandSyntaxException {
            ...
            throw new DynamicCommandExceptionType(value -> Text.translatable(
                            "command.tempus.nutrition.invalid", value.toString().toLowerCase()))
                    .create(maybeType);
        }
